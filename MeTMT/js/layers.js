addLayer("+", {
    ...
    tabFormat: {
		...
		["display-text",
		    function() { if (player['+'].points.gte(28)) {return "- +2 Upgrades for <span style=\"color: rgb(255, 0, 0); text-shadow: rgb(255, 0, 0) 0px 0px 10px;\">multiplier</span>, Unlock Challenges, x777 <span style=\"color: rgb(255, 0, 255); text-shadow: rgb(255, 0, 255) 0px 0px 10px;\">ultra</span> points, omega and inf} },],
                ["display-text",
...
addLayer("p", {
	...
	if (hasUpgrade('x', 15)) mult = mult.times(5)
        return mult
    },
    automate() {
        if (hasUpgrade('r', 16)) {
...
addLayer("r", {
	...
	if (hasUpgrade('x', 16)) mult = mult.times(5)
        return mult
    },
    gainExp() { // Calculate the exponent on main currency from bonuses
        return new Decimal(1)
